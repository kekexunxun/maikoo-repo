...
import com.maikoo.businessdirectory.dao.ClassUserDao;
...
@Transactional
@Service
public class ClassGroupService extends AbstractGroupService {
    @Autowired
    ...
    public GroupDTO insert(GroupInformationQuery groupInformationQuery) {
        ...
        ClassGroupInformationQuery classGroupInformationQuery = asClassGroupInformationQuery(groupInformationQuery);
        ClassGroupDO classGroupDO = ClassGroupDO.valueOf(classGroupInformationQuery);
        classGroupDO.setUserDO(currentUserDO);

        String key = redisUtil.groupSN(GroupTypeEnum.CLASS, currentUserDO.getUserId());
        redisTemplate.opsForValue().set(key, classGroupDO, 10 * 60, TimeUnit.SECONDS);
    ...
    public void update(GroupInformationQuery groupInformationQuery) {
        ...
        ClassGroupInformationQuery classGroupInformationQuery = asClassGroupInformationQuery(groupInformationQuery);
        ClassGroupDO classGroupDO = ClassGroupDO.valueOf(classGroupInformationQuery);
        classGroupDao.update(classGroupDO);
    }
    ...
    public void remove(long id) {
        ...
        int dismissResult = classGroupDao.dismiss(id);

        //通过解散群的id获取userIdList,获取群名称
        String groupName = classGroupDao.getGroupName(id);
        List<Long> ids = classUserDao.selectUserIdByGroupId(id);
    ...
    public void changeOwner(GroupQuery groupQuery) {
        ...
        UserDO currentUserDO = (UserDO) session.getAttribute("current_user");
        int count = classGroupDao.changeOwner(groupQuery.getUserId(), groupQuery.getGroupId());
        if (count > 0) {
            //添加站内消息
            String groupName = classGroupDao.getGroupName(groupQuery.getGroupId());
            String userName = classUserDao.getUserName(currentUserDO.getUserId(), groupQuery.getGroupId());
            List<MessageDO> messageDOList = new ArrayList<>();
            MessageDO messageDO = new MessageDO();
            messageDO.setMsgTitle("系统通知");
            ...
            messageDOList.add(messageDO);
            //发站内消息给自己
            String acceptName = classUserDao.getUserName(groupQuery.getUserId(), groupQuery.getGroupId());
            ...
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    UserDO userDO = userDao.selectOne(userId);
                    ClassGroupDO classGroupDO = classGroupDao.selectOne(groupId);
                    ClassUserDO classUserDO = classUserDao.selectIdxByGroupIdAndUserId(groupId, userId);

                    templateMessageUtil.changeOwner(userDO, classGroupDO.getGroupName(), classUserDO.getName());
                }
            });
            thread.start();
        }
    ...
    public PostDTO sharePost(long id) {
        ClassGroupDO classGroupDO = classGroupDao.selectOne(id);

        if (StringUtils.isEmpty(classGroupDO.getPosterUrl()) || StringUtils.isEmpty(classGroupDO.getQrCodeUrl())) {
            try {
                ...
                BufferedImage qrCodeBufferedImage = wechatUtil.qrCode(GroupTypeEnum.CLASS, id);

                classGroupDO.setQrCodeUrl("/" + fileUtil.saveImage(qrCodeBufferedImage, "PNG"));
                classGroupDO.setPosterUrl("/" + imageLocation + fileUtil.filename(baseLocation + imageLocation, "PNG"));

                PosterQuery posterQuery = PosterQuery.valueOf(classGroupDO);
                posterQuery.setQrCodeBufferedImage(qrCodeBufferedImage);

                posterUtil.classPoster(posterQuery);

                classGroupDao.updateShareUrl(classGroupDO);
            } catch (Exception e) {
        ...
        postDTO.setPosterUrl(classGroupDO.getPosterUrl());
        postDTO.setQrcodeUrl(classGroupDO.getQrCodeUrl());
        return postDTO;
    }
    ...
    public GroupDTO information(long id) {
        ...
        ClassGroupDO classGroupDO = classGroupDao.selectOne(id);
        if (classGroupDO != null) {
            List<Long> ids = classUserDao.selectIdsByGroupId(classGroupDO.getGroupId());
            classGroupDO.setGroupMemCount(CollectionUtils.isEmpty(ids) ? 0 : ids.size());
            groupDTO = GroupDTO.valueOf(classGroupDO);
            if (classGroupDO.getUserDO().getUserId() == currentUserDO.getUserId()) {
                groupDTO.setMemType(UserRoleEnum.ADMIN);
            } else if (classUserDao.selectIdxByGroupIdAndUserId(classGroupDO.getGroupId(), currentUserDO.getUserId()) != null) {
    ...
    public GroupDTO informationByAdmin(long id) {
        ...
        if (classGroupDO != null) {
            groupDTO = GroupDTO.valueOf(classGroupDO);
            groupDTO.setGroupAddress(addressService.address(Lists.newArrayList(classGroupDO.getGroupAddrCode().split("_"))) + classGroupDO.getGroupAddrDetail());
        }
        return groupDTO;
    }
    ...
    public List<GroupDTO> selectAllByAdmin() {
        ...
        List<ClassGroupDO> classGroupDOList = classGroupDao.selectAll();

        if (!CollectionUtils.isEmpty(classGroupDOList)) {
            classGroupDOList.forEach(classGroupDO -> {
                GroupDTO groupDTO = GroupDTO.valueOf(classGroupDO);
                groupDTO.setGroupAddress(addressService.address(Lists.newArrayList(classGroupDO.getGroupAddrCode().split("_"))) + classGroupDO.getGroupAddrDetail());
    ...
    public FileDTO excel() {
        ...
        if (!CollectionUtils.isEmpty(classGroupDOList)) {
            ...
            baseRow.createCell(9).setCellValue("班级名称");

            IntStream.range(0, classGroupDOList.size()).forEach(idx -> {
                Row row = sheet.createRow(idx + 1);

                ClassGroupDO classGroupDO = classGroupDOList.get(idx);
                String createdAt = LocalDateTime.
                        ofInstant(Instant.ofEpochSecond(Long.valueOf(classGroupDO.getCreatedAt())), ZoneId.of("UTC+08:00")).
                        format(DateTimeFormatEnum.COMMON.getDateTimeFormatter());
                String dismissedAt = classGroupDO.isEnable() ? "" : LocalDateTime.
                        ofInstant(Instant.ofEpochSecond(Long.valueOf(classGroupDO.getDismissedAt())), ZoneId.of("UTC+08:00")).
                        format(DateTimeFormatEnum.COMMON.getDateTimeFormatter());
                row.createCell(0).setCellValue(classGroupDO.getGroupId());
                row.createCell(1).setCellValue(classGroupDO.getGroupName());
                row.createCell(2).setCellValue(addressService.address(Lists.newArrayList(classGroupDO.getGroupAddrCode().split("_"))) + classGroupDO.getGroupAddrDetail());
                row.createCell(3).setCellValue(classGroupDO.getGroupBrief());
                row.createCell(4).setCellValue(classGroupDO.getGroupMemCount());
                row.createCell(5).setCellValue(createdAt);
                row.createCell(6).setCellValue(dismissedAt);
                row.createCell(7).setCellValue(classGroupDO.isEnable() ? "ENABLE" : "DISMISS");
                row.createCell(8).setCellValue(classGroupDO.getSchoolName());
                row.createCell(9).setCellValue(classGroupDO.getClassName());
            });
            LocalDate localDate = LocalDate.now();
            String basePath = customEnvironmentConfig.getUploadLocation() + customEnvironmentConfig.getExcelLocation();
            String filename = fileUtil.filename(basePath, "xls", localDate.format(DateTimeFormatEnum.COMMON_DATE.getDateTimeFormatter()) + "-" + "同班群-");
    ...
    private ClassGroupInformationQuery asClassGroupInformationQuery(GroupInformationQuery groupInformationQuery) {
        if (!(groupInformationQuery instanceof ClassGroupInformationQuery)) {
            throw new ClassCastException();
        }
        return (ClassGroupInformationQuery) groupInformationQuery;
    }
    ...
    private void groupAdmin(long groupId) {
        ...
        if (classGroupDao.isExistedByUserIdAndGroupId(currentUserDO.getUserId(), groupId) == null) {
