...
Page({
  ...
  onLoad: function(options) {
    // 页面显示
    var that = this;
    let activeIndex = options.state ? options.state : 0;
    wx.getSystemInfo({
  ...
  onShow: function() {
    ...
    that.getOrderList(orderList[activeIndex].state, 0);
  },
  ...
  confirmOrder: function(evt) {
    ...
    util.modalPromisified({
      ...
      util.post('order/confirmOrderGoods', {
        ordersn: evt.currentTarget.dataset.ordersn,
        uid: app.globalData.uid
      }).then(res => {
        wx.showToast({
          title: '确认收货成功',
        ...
        setTimeout(res => {
          let orderList = that.data.orderList;
          orderList[that.data.activeIndex].pageNum = 0;
          orderList[that.data.activeIndex].list = [];
          that.setData({
            orderList: orderList
          })
          that.getOrderList(orderList[that.data.activeIndex].state, orderList[that.data.activeIndex].pageNum);
        }, 1200)
      }).catch(res => {
        util.modalPromisified({
          title: '系统提示',
