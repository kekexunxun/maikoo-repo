...
class Goods extends Controller
{
    ...
    public function getGoodsData(Request $request)
    {
        ...
        // dump($mchId);die;
        // 调用公共函数
        $goodsInfo = getGoods($field, $mchId, true);
        // dump($goodsInfo);die;
        // 查询审核与上下架的权限信息
    ...
    public function goodsedit()
    {
        ...
        // 二级分类信息
        $catagoryTwo = $catagory->field('cat_id,cname,parent_id')->where('parent_id', 1)->where('status', '<>', 2)->select();
    ...
    public function editGoods(Request $request)
    {
        ...
        $where['market_price'] = intval($request->param('market_price'));
        $where['shop_price'] = intval($request->param('shop_price'));
        $where['member_price'] = intval($request->param('member_price'));
    ...
    public function addGoodsPic(Request $request)
    {
        ...
        if ($info) {
            ...
            $goodssrc = 'imgTemp' . DS . $str2;
            return json($goodssrc);
        }
    }
    ...
    {
        $catagory = new Catagory;
        $res = $catagory->field('cat_id,parent_id,cname')->where('status', 1)->order('sort desc')->select();
        if ($res) {
            // 构造返回数组
...
// ******************
    ...
    public function columnadd(Request $request)
    {
        if ($request->post()) {
            ...
            $ids = Session::get('goodsIds');
            // 对字符串处理转为数组
            $ary = explode(',', $ids);
            foreach ($ary as $key => $value) {
                if (intval($value) > 1000000) {
                    $goods[] = intval($value) - 1000000;
                }
            }
            // dump($goods);die;
            //专栏封面图片存储目录
    ...
    public function preCatGoods(Request $request)
    {
        ...
        $catagory = new Catagory;
        $catList = $catagory->field('cat_id,parent_id,cname')->where('status', 1)->order('sort desc')->select();
        if (!$catList || count($catList) == 0) return objReturn(400, 'No Cat');
        $catList = collection($catList)->toArray();
        // 专栏的商品信息
        $columnGoods = new Column_goods;
        $goodsList = $columnGoods->field('goods_id')->where('status', 1)->where('column_id', $columnId)->select();
        if (!$goodsList || count($goodsList) == 0) return objReturn(400, 'No CatGoods');
        $goodsList = collection($goodsList)->toArray();
        $goodsIds = [];
