...
class Cart extends Controller
{
    ...
    public function getCart()
    {
        ...
        // 判断是否有商品无效
        $goods = new Goods;
        $goodsList = $goods->where('goods_id', 'in', $goodsIdx)->where('status', 2)->field('goods_id, goods_sn, goods_name, goods_img, market_price, shop_price, sales_num, keywords, unit, points, status')->order('sort desc')->select();
        ...
        foreach ($cartGoodsList as &$cartGoods) {
            foreach ($goodsList as &$good) {
                if ($cartGoods['goodsid'] == $good['goods_id']) {
                    $isAllGoodsFixed[] = 1;
                    $good['keywords'] = str_replace(',', ' ', $good['keywords']);
