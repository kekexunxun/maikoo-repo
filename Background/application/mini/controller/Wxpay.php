...
class Wxpay extends Controller
{
     ...
     * @return json result
     */
    public function createWxPay(Request $request)
    {
        $openid = $request->param('openid');
        $orderSn = $request->param('ordersn');
        $uid = intval($request->param('uid'));
        // 数据验证
        if (empty($openid) || empty($orderSn) || empty($uid)) return objReturn(401, 'Invaild Param');

        // 订单缓存查找
        $orderPrepayCache = $this->checkPrepay($orderSn);
        if ($orderPrepayCache) return objReturn(0, 'Get cache Success', $orderPrepayCache);
        
        // 若没有对应缓存 此时就相当于重新下单
        $order = new Order;
        $totalFee = $order->where('order_sn', $orderSn)->value('total_fee');
    ...
    public function checkWxpay()
    {
        ...
        foreach ($prepayCache as $k => $v) {
            if ($v['orderSn'] == $orderSn && $v['isChecked']) {
                $isChecked = true;
        ...
        if ($isChecked) {
            ...
            // 修改订单状态
            $order = new Order;
            $order->where('order_sn', $orderSn)->update(['uid' => $uid, 'pay_at' => time(), 'status' => 2]);
            ...
            foreach ($prepayCache as $k => $v) {
                if (isset($v['orderSn']) && $v['orderSn'] == $orderSn) {
                    if ($v['order_expire_time'] < time()) {
                        $order = new Order;
                        $order->where('order_sn', $orderSn)->update(['status' => 6, 'cancel_at' => time()]);
                        $isExpire = $k;
                        break;
                    } else {
                        if (!$prepayCache[$k]['isCache']) {
    ...
    public function unifiedOrder($payInfo, $timeOut = 6)
    {
        ...
        // 设置过期时间为 1180s
        $result['order_expire_time'] = intval(time() + 3600);
    ...
    public function checkWxPayResult()
    {
        ...
        if ($data['return_code'] == 'SUCCESS' && $data['result_code'] == 'SUCCESS') {
            ...
            // 从订单中获取部分字段信息
            $totalFee = Db::name('order')->where('order_sn', $data['out_trade_no'])->value('total_fee');
