...
class User extends Controller
{
    ...
    public function getUserFav()
    {
        ...
        $user_fav = new User_fav;
        $userFavList = $user_fav->alias('f')->join('sm_goods g', 'f.goods_id = g.goods_id', 'LEFT')->field('f.fav_id, g.goods_id, g.goods_name, g.goods_img, g.shop_price, g.market_price, g.keywords, g.unit')->where('f.uid', $uid)->where('g.status', 2)->limit($pageNum * 10, 10)->select();
