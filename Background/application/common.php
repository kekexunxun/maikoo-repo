...
function getGoods($field = null, $mchId = null, $isAll = true, $pageNum = null)
{
    ...
    if (isset($mchId)) {
        ...
        $goodsList = $goods->alias('g')->join('sm_merchant_stock s', 'g.goods_id = s.goods_id', 'RIGHT')->where('s.mch_id', $mchId)->field($field)->select();
    } else {
    ...
    foreach ($goodsList as &$good) {
        ...
        if (isset($good['goods_img'])) $good['goods_img'] = "https//store.up.maikoo.cn/static" . $good['goods_img'];
    }
    return $goodsList;
}
...
function getColumn($field = null, $isAll = true, $status = null)
{
    ...
    foreach ($columnList as &$info) {
        ...
        if (isset($info['column_name'])) $info['column_name'] = htmlspecialchars_decode($info['column_name']);
    }
    return $columnList;
}
...
function getColumnById($columnId, $isAll = true, $goodsLimit = null)
{
    ...
    $columnGoodsList = $column_goods->alias('c')->join('sm_goods g', 'c.goods_id = g.goods_id', 'LEFT')->where('c.column_id', $columnId)->where('c.status', 'in', $columnGoodsStatus)->field('c.idx, c.goods_id, c.sort, c.status, g.goods_sn, g.goods_name, g.goods_img, g.market_price, g.shop_price, g.keywords')->order('c.sort desc')->limit($columnGoodsNumLimit)->select();
...
function getOrder($status = 0, $pageNum = null, $uid = null, $mch_id = null, $orderSn = null)
{
    ...
    $orderField = 'order_id, order_sn, uid, mch_id, total_fee, logi_fee, coupon_fee, username, phone, address, message, pay_at, created_at, finish_at, cancel_at, status, mch_name, delivery';
    if (isset($uid)) {
        $orderList = $order->where('uid', $uid)->where('status', 'in', $status)->field($orderField)->limit($limit)->select();
    } else if (isset($mch_id)) {
        $orderList = $order->where('mch_id', $mch_id)->where('status', 'in', $status)->field($orderField)->limit($limit)->select();
    } else if (isset($orderSn)) {
        $orderList = $order->where('order_sn', $orderSn)->where('status', 'in', $status)->field($orderField)->select();
    } else {
        $orderList = $order->where('status', 'in', $status)->field($orderField)->limit($limit)->select();
    }
    ...
    foreach ($orderList as &$info) {
        ...
        $detail = $order_detail->alias('d')->join('sm_goods g', 'd.goods_id = g.goods_id', 'LEFT')->where('d.order_id', $info['order_id'])->field($detailField)->select();
        $detail = collection($detail)->toArray();
        $info['quantity'] = 0;
        foreach ($detail as $k => $v) {
            $detail[$k]['goods_img'] = $baseUrl . $v['goods_img'];
            $detail[$k]['goods_name'] = htmlspecialchars_decode($v['goods_name']);
