...
function courseadd_save_submit() {
    ...
    $('input[name="sort_id"]:checked').each(function () {
        sort = $(this).parent().parent().parent().find('.course_img_sort').val();
        select += $(this).val() + ":" + sort + ",";//向数组中添加元素
        // console.log(select);
    });
    // 发送ajax
    $.ajax({
        ...
        url: '../../editCourse',
        dataType: 'json',
        data: $.param({ 'img_url': select }) + '&' + $("#form-course-add").serialize(),
...
(function ($) {
    ...
    $(function () {
        ...
        uploader = WebUploader.create({
            ...
            accept: {
                title: 'Images',
                extensions: 'jpg,png',//可选gif,jpg,jpeg,bmp,png
                mimeTypes: 'image/*'
            },
        ...
        function addFile(file) {
            var arr = file.name.split('\\');//注split可以用字符或字符串分割
            var fileName = arr[arr.length - 1];//这就是要取得的名文件称
            var index = fileName.indexOf(".");
            var isNum = fileName.substring(0, index);
            if (isNaN(isNum)) {
                layer.alert('图片命名不正确！', { icon: 2 });
